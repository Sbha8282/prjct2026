...
class AssignWorker(LoginRequiredMixin, View):
    ...
    def post(self,request,id):
        ...
        req = ServiceRequests.objects.get(id=id)
        if not worker:
            workers_records = workers.objects.filter(designation=req.service.Name)
            context = {
                'req': req,
                'workers_records': workers_records,
                'msg': 'Please select a valid worker before submitting.',
            }
            return render(request, 'adminpages/assign_worker.html', context)
...
class userprofile(LoginRequiredMixin, View):
    ...
    def get(self,request):
        ...
        data = users.objects.get(admin=admin_user)
        user_requests = ServiceRequests.objects.filter(user=data)
        assigned_responses = Response.objects.filter(requests__user=data).select_related('requests__service', 'assigned_worker__admin')
        pending_services = assigned_responses.filter(status=False)
        completed_services = assigned_responses.filter(status=True)
        unassigned_requests = user_requests.filter(response__isnull=True).count()
        ...
        context = {
            ...
            'unassigned_count': unassigned_requests,
...
def book_service(request):
    return render(request, 'userpages/servicebook.html')

def my_orders(request):
    return render(request, 'userpages/appointment_history.html')
...
def assigned_jobs(request):
    return render(request, 'workerpages/view_request.html')
...
def admin_dashboard(request):
    return render(request, 'adminpages/adminhomepage.html')
